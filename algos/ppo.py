...
def create_training_data(episodes, single_raise=True, value_function=None):
    """
    ...
    if value_function:
        with torch.no_grad():
            qs = value_function(observations, return_sequences=True)[0]
    else:
        qs = None
    return {key: value for key, value in locals().items() if key in [
        "observations", "action_types", "betfracs", "masks", "action_masks", "rewards", "legal_actions_mask", "qs"
    ]}
...
class Agent:
    ...
    def get_action(self, game, smallest_unit=1, temperature=1):
        return self.policy.get_action(game, smallest_unit, temperature=temperature)
    ...
    def generate_training_data(self):
        ...
        self.training_data = create_training_data(self.episodes, value_function=value_function)
    ...
    def reset(self):
        ...
        self.blind=0
    def train_policy(self, epochs=4, clip_epsilon=0.2, entropy_coef=0.01, batch_size=64, verbose=True):
        """
        ...
        if verbose:
            print(f"Starting PPO training for {epochs} epochs with batch size {batch_size}...")
        ...
        if verbose:
            # Freeze the old policy (once per training cycle)
            print("Creating a copy of the old policy for this training cycle...")
        ...
        for epoch in range(epochs):
            ...
            if verbose:
                print(f"  Avg Policy Loss: {avg_policy_loss:.4f}, Avg Entropy Loss: {avg_entropy_loss:.4f}")
        self.policy.eval()
...
def check_hand_evals(agent, output_file="hand_evaluations.csv", new_game=None):
    ...
    for rank in ranks:
        for suit1, suit2 in combinations(suits, 2):
            ...
            observations_test.append(state_vector.unsqueeze(dim=0))
    ...
    if not agent.policy.single_raise:
        columns += ["Minbet Prob", "All-In Prob"]
