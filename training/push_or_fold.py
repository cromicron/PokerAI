...
from Agents.PushFoldAgent import Agent
...
for k in tqdm.tqdm(range(200000)):
    ...
    while not game.finished:
        ...
        legal_actions = [0, 2] if position == 0 else [0, 1]
        action = agent_to_act.choose_action(
            cards,
            position,
            legal_actions,
        )
        ...
        episode = map_episode[agent_to_act]
        hand_rep = agent_to_act.one_hot_encode_hand(*cards)
        observation = np.array([[hand_rep[0], position]])
    ...
    for player, agent in map_agent.items():
        ...
        reward = reward + 1 if game.positions.index(player) == 0 else reward + 2
        if len(map_episode[agent])==2:
            agent.add_observation_to_q_buffer(map_episode[agent][0], map_episode[agent][1], reward)
