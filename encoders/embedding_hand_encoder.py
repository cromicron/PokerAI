...
intermediary_dim = 16
module = PokerHandEmbedding(embedding_dim, feature_dim, deep_layer_dims, intermediary_dim)
checkpoint = torch.load(MODEL_SAVE_PATH)  # Load the checkpoint file
module.load_state_dict(checkpoint["model_state_dict"])  # Load model weights
module.to("cpu")
module.eval()

@torch.no_grad()
def encode(holecards, flop=None, turn=None, river=None):
    """
    ...
    try:
        ...
        features, log_probs = module(holecards, flop, turn, river)
