...
import numpy as np
import itertools
from handstrength import compare
agent="test"
...
class HUPoker:   
        def __init__(self, stack_hero, stack_villain,n_run =1, hero=agent): 
            ...
            self.bet_sb = 0.5 #keeps track of the bet chips by player in current


        def reset(self): #shuffles the deck, returns position, stack sizes, hand (only suited or unsuited, because actual stuits are not important preflop) 
            ...
            self.bet_bb = 1
            self.bet_sb = 0.5
            self.stack_sb = self.starting_hero - 0.5 if self.position == 0 else self.starting_villain - 0.5
            self.stack_bb = self.starting_hero - 1 if self.position == 1 else self.starting_villain -1
        ...
        def implement_action(self, action, player = 'hero'): #implements either heros or villains action to the point where either the next player is to act or the hand is finished
            ...
            self.hh[self.street][self.count_round] = bet
            return done       
        def implement_action_player(self, betsize, player = 'villain'): #implements user input.
            #betsize is the total bet in the userinput
                    action = "bet"
                    bet = 0
                    self.count_round += 1
                    done =False
                    ...
                    #player cannot raise, if he has less than pot - self.bet
                    if  (self.position == 1 and self.bet_bb - self.bet_sb >= self.stack_sb) or (
                        self.position == 0 and self.bet_sb - self.bet_bb >= self.stack_bb):
                        action = "call"


                    
                    if action == "call": #player calls/checks
                        if next_to_act == 0: #player is sb
                            if self.bet_sb == self.bet_bb: #bets are ballanced, so player can check.
                        ...
                        # if unbet the bet variable indicates how much the player will increase his current betsize.
                        bet = betsize - self.bet_bb if self.position == 0 else betsize - self.bet_sb
