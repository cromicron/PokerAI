...
def strength(hand): #One after the other we are going to check for hand strength. The return satatement will end the function and give back the hand strength type
    ...
    if isType:
        strength_val = 5+1e-2*substrength[0]+1e-4*substrength[1]+1e-6*substrength[2]+1e-8*substrength[3]+1e-10*substrength[4]
        return strength_val
    ...
    return 0+ 1e-2*highcards[0]+1e-4*highcards[1]+1e-6*highcards[2]+1e-8*highcards[3]+1e-10*highcards[4]
