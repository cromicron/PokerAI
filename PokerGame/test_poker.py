...
from NLHoldem import SizeError
p = os.path.abspath('/home/cromi/Documents/Code/Python/PokerAI/Training')
sys.path.insert(1, p)
...
class TestPoker(unittest.TestCase):
    ...
    def test_comperator(self):
        for i in range(1000):
            ...
            community_lst = [(card.value,card.suit) for card in community]
            hole_0_lst = [(hole_0[0].value, hole_0[0].suit),(hole_0[1].value, hole_0[1].suit)]
            hole_1_lst = [(hole_1[0].value, hole_1[0].suit),(hole_1[1].value, hole_1[1].suit)]
            hand_0_lst = hole_0_lst + community_lst
            hand_1_lst = hole_1_lst + community_lst
            dct_comp = compare({0:hand_0_lst,1:hand_1_lst})
            func_comp = compare_hands([hole_0+community, hole_1+community])
            if func_comp[0] > func_comp[1]:
                dct_func = {0:0, 1:1}
            elif func_comp[1] > func_comp[0]:
                dct_func = {1:0, 0:1}
            else:
                dct_func = {0:0, 1:0}
            self.assertDictEqual(dct_comp, dct_func)



    def test_compare_mult(self):
        from NLHoldem import Card
